...
func ParseCertSerialNo(data []byte) (string, int32) {
	if data == nil {
	...
	return cert.SerialNumber.String(), errutil.Succ
}

func LoadCertSerialNo(certPath string) (string, int32) {
...
func Sign(priKey *rsa.PrivateKey, data []byte) (string, int32) {
	bs, err := rsa.SignPKCS1v15(nil, priKey, crypto.SHA1, SHA1Digest(data))
	if err != nil {
		return "", errutil.Sys_UnKnown
	}

	return base64.StdEncoding.EncodeToString(bs), errutil.Succ
}

func Verify(pubKey *rsa.PublicKey, data []byte, sign string) int32 {
...
func VerifyRSAWithMD5(pubKey *rsa.PublicKey, data []byte, sign string) int32 {
	bs, err := base64.StdEncoding.DecodeString(sign)
	if err != nil {
		return errutil.Sys_UnKnown
	}
